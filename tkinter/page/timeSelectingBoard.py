    ...
    def selectAll(self):
        if usingLocalTime:
            for index in range(0, 24, 2):
                self.buttonHours[index].configure(background=backgroundColors[1])
            ...
            write_multiple_registers(DeviceAddr.selectingHoursAddr.value, [1 for i in range(0, 24, 2)], 
                lambda rec:[self.buttonHours[index].configure(background=backgroundColors[1]) for index in range(0, 24, 2) ] 
                    and setattr(deviceController,'selectingHours',[1 for i in range(0, 24, 2)]) , repeatTimes = 0 , needMesBox = True)
    # 
    def resetAll(self):
        if usingLocalTime:
            for index in range(24):
                self.buttonHours[index].configure(background=backgroundColors[0])
